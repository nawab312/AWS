...
- **Response Headers** Response headers in ALB are the HTTP headers that are sent back to the client in the response. They may be generated by the ALB itself (like during a redirect), added by ALB (like security or CORS headers), or simply forwarded from the backend server.
  - Generated by the ALB itself (like during a redirect): ALB creates the response on its own without asking your backend server.
    - Example, User types: `http://example.com`
    - Your Application Load Balancer is configured to force HTTPS.
    - So what happens? Request reaches ALB. ALB does NOT send it to your EC2 or app. ALB directly replies:
      ```bash
      HTTP/1.1 301 Moved Permanently
      Location: https://example.com
      ```
    - That `Location` header was created by ALB. Your backend was never involved. That is what “generated by ALB” means.
      ```bash
      Request headers → Client → ALB
      Response headers → ALB → Client
      ```
  - Added by ALB (like security or CORS headers): Your backend did not send those headers. ALB attaches them before sending the response to the client.
    - Example: Your backend sends this:
      ```bash
      HTTP/1.1 200 OK
      ...
      { "status": "success" }
      ```
    - Now your Application Load Balancer is configured to add a security header. So the client actually receives:
      ```bash
      HTTP/1.1 200 OK
      Content-Type: application/json
      ...
      { "status": "success" }
      ```
  - Simply forwarded from the backend server: ALB did not create the header. ALB did not change the header. ALB just passed it along to the client.
