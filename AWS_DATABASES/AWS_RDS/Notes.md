...
- *Security Groups and NACLs*
  - Security Groups: Allow access only from trusted EC2 instances or IPs. Restrict traffic to specific ports (e.g., MySQL - 3306, PostgreSQL - 5432).
  - Network ACLs: Additional layer of security to control subnet-level traffic.
- *Public Accessibility*
  - Disable public access (`Publicly Accessible = No`): Unless explicitly required.

**Authentication & Access Control**
- *IAM Authentication*
  - IAM users and roles can authenticate using temporary credentials instead of traditional usernames and passwords.
- *Database Authentication*
  - Native Database Authentication: Use MySQL, PostgreSQL, or SQL Server authentication.
  - AWS IAM Authentication: Uses IAM policies to grant access.

**Encryption**
- *Encryption at Rest*
  - AWS KMS (Key Management Service) Encryption:
    - Enable storage encryption for RDS instances.
    - Encrypt database snapshots, backups, and read replicas.
    - Encryption cannot be enabled on an existing RDS instance (requires a new instance with encryption enabled).
- *Encryption in Transit*
  - Use SSL/TLS to encrypt data transmission between the database and clients.
  - Download the RDS SSL certificate and enforce SSL/TLS connections.

**Auditing & Monitoring***
- CloudTrail Logging
  - Captures API calls related to RDS instances for security auditing.
  - Logs actions like instance creation, deletion, and modifications.
- Enhanced Logging
  - RDS Database Activity Streams: Captures real-time database activity.
  - CloudWatch Logs & Alarms:
    - Monitor database performance and security metrics.
    - Set up alarms for unauthorized access attempts.
- AWS Config
  - Continuously monitors RDS configurations and changes.
...
- *Manual Snapshots*
  - Create manual snapshots. Store snapshots in Amazon S3 or another AWS region.
- *Cross-Region Replication*
  - Use Read Replicas or AWS Backup for disaster recovery across regions.
  - Schedule *maintenance windows* for applying updates.
 
**DDoS Protection**
- AWS Shield and AWS WAF can protect RDS from large-scale attacks.
- Use *AWS Route 53 DNS Failover* to switch traffic in case of an attack.

**Secure Application Access**
- Use *Secrets Manager* or *SSM Parameter Store* to store and retrieve database credentials securely.
